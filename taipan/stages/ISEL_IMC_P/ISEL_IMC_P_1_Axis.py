...
class ISEL_IMC_P_1_Axis_X(Manipulator):
 ...
 #   class ActiveLevelLimitSwitch2(enum.Enum):
  #      LowActive = 0
   #     HighActive = 1
    ...
    def __init__(self, comport, baudRate=19200, stepsPerRev=135, objectName=None, loop=None):
    ...
    def query(self, command,timeout=None):
        ...
        with self._lock:
            ...
            answerStringFirstStage = str(answerString[0:6])
            answerStringSecondStage = str(answerString[6:12])
            print('ISEL_IMC_P: answer First Stage(hex): ' + answerStringFirstStage)
            print('ISEL_IMC_P: answer Second Stage(hex): ' + answerStringSecondStage)
            if timeout is not None:
