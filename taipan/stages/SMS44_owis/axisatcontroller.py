...
class AxisAtController_SMS44(TMCL):
    ...
    def __init__(self, port, objectName=None, loop=None):
        ...
        self.manipConvFactor = 0.00279125
    ...
    async def _set_important_parameters(self, store=False):
        ...
        importantParams = {"max_current": (6, 60), "max_speed": (4, 200), "standbycurrent": (7, 8),
                           "max_accel": (5, 1000), "right_limit_switch_disable": (12, 0),
                           "left_limit_switch_disable": (13, 0), "microstep_resolution": (140, 4),
                           "ref_search_mode": (193, 1),
                           "ref_search_speed": (194, 200)}
    ...
    async def reference(self):
        ...
        # set 0 at 1 unit away from limit switch, otherwise 0 position triggers endswitch and it never reaches 0
        await asyncio.sleep(0.50)
        await self.moveTo(Q_(1, self.unit))
        await self.resetCounter()
